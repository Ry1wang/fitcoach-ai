...
| ------------------------------ | ----------------- | ----- | ------------------------------------------------------------ |
| `cache:query:{md5_hash}`       | JSON string       | 3600s | Cached agent response (hash of normalized query). Only first-message (context-free) queries are cached. |
...
class CacheService:
    ...
    def _make_key(self, query: str) -> str:
        """Cache key is a hash of the normalized query only.
        Caching is restricted to first-message (context-free) queries,
        so conversation history is intentionally excluded."""
...
async function generate_stream(request, user, session, cache):
    ...
    if is_first_message:
        await cache.set(request.message, {
...
```
                  ...
                  └────────────┬─────────────┘
                    ...
                    YES  │           │ NO (multi-turn)
                         ▼           ▼
               ┌──────────────┐  ┌──────────────┐
               │ Hash(query)  │  │ Skip cache — │
               │ Redis Lookup │  │ run agent    │
               └──┬────────┬──┘  │ pipeline     │
                  │        │     └──────────────┘
             HIT  │        │ MISS
                  ▼        ▼
...
```
cache:query:{md5(normalized_query)}
...
Caching scope:
  - Only first-message (context-free) queries are cached
  - Multi-turn queries are excluded because identical questions in
    different conversation contexts should produce different answers
  - Cached responses are wrapped in SSE events at read time, so the
    client always receives a consistent stream format
...
async def test_cache_miss_then_hit(redis_client):
    ...
    # Set
    await cache.set("test query", {"response": "cached answer"})
...
async def test_cache_key_normalization(redis_client):
    ...
    await cache.set("  Hello World  ", {"data": 1})
