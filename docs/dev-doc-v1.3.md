...
> **Author:** Hush  
> **Repository:** github.com/[TBD]/fitcoach-ai
...
| LLM Provider API      | LLM inference (chat completion) and text embedding generation. Provider-agnostic via OpenAI-compatible SDK. Natively supports DeepSeek and OpenAI; Claude requires a compatibility proxy (LiteLLM). | External service                      |
...
| Cache / Rate Limit  | Redis                           | 7 (Alpine)       | In-memory key-value store; low latency for cache-aside pattern and fixed-window rate limiting |
...
| LLM Provider        | OpenAI-compatible API           | —                | Provider-agnostic design via OpenAI SDK; natively supports DeepSeek and OpenAI. Claude requires a compatibility proxy (LiteLLM) since Anthropic's API is not OpenAI-compatible. Chat and embedding providers can be configured independently — switch by changing `.env` only, no code changes |
...
class Settings(BaseSettings):
    ...
    EMBEDDING_MODEL: str = "text-embedding-v1"   # DeepSeek default; verify via their docs
...
> The LLM (chat) and Embedding providers are configured independently because not all LLM providers offer embedding endpoints. For example, Claude does not provide an embedding API, so a deployment using Claude for chat may use OpenAI or another provider for embeddings. The `EMBEDDING_*` fields fall back to `LLM_*` values when not set, so single-provider setups (e.g., DeepSeek for both chat and embedding) require no extra configuration.
...
EMBEDDING_MODEL=text-embedding-v1       # DeepSeek default; verify via their docs
...
LLM_BASE_URL=https://api.deepseek.com/v1
LLM_CHAT_MODEL=deepseek-chat
EMBEDDING_MODEL=text-embedding-v1       # Verify model name via DeepSeek docs before use
EMBEDDING_DIMENSION=1024
...
# NOTE: Anthropic's native API is NOT OpenAI-compatible (different auth
# headers and request/response schemas). To use Claude via the OpenAI SDK,
# you must run a compatibility proxy such as LiteLLM:
#   pip install litellm && litellm --model claude-sonnet-4-20250514 --port 4000
# Then point LLM_BASE_URL at the proxy:
LLM_API_KEY=sk-ant-xxx
LLM_BASE_URL=http://localhost:4000/v1   # LiteLLM proxy, NOT api.anthropic.com
...
> **Important — Embedding Model Verification:**
> The default `EMBEDDING_MODEL` value assumes a DeepSeek embedding endpoint. Before first use, verify the exact model name via [DeepSeek's API documentation](https://platform.deepseek.com/api-docs). If unavailable, switch to OpenAI's `text-embedding-3-small` by setting `EMBEDDING_BASE_URL` and `EMBEDDING_API_KEY` to OpenAI values.

> **Important — Claude / Anthropic Compatibility:**
> Anthropic's native API uses different authentication headers (`x-api-key` instead of `Authorization: Bearer`) and different request/response schemas than the OpenAI SDK expects. You cannot point the OpenAI SDK directly at `https://api.anthropic.com`. Use a compatibility proxy like [LiteLLM](https://docs.litellm.ai/) to translate between the two formats.
...
│ created_at   │   │   │ file_size        │       │ chunk_type      │
│ is_active    │   │   │ domain           │       │ metadata (JSONB)│
...
| username        | VARCHAR(50)  | NOT NULL                      | Display name (not unique — multiple users may share the same display name; login uses email) |
...
| user_id       | UUID         | FK → users.id, ON DELETE CASCADE | Document owner. Cascade: deleting a user removes all their documents (and transitively all chunks via documents cascade). |
| filename      | VARCHAR(255) | NOT NULL          | Original filename                                            |
| file_path     | VARCHAR(500) | NOT NULL          | Server-side storage path                                     |
| file_size     | INTEGER      | —                 | File size in bytes                                           |
| domain        | VARCHAR(50)  | —                 | Knowledge domain classification: 'training', 'rehab', 'nutrition'. Propagated to chunk metadata as `content_domain` during ingestion. |
...
| chunk_type  | VARCHAR(50)  | —                                    | Structural content classification: 'text', 'table', 'exercise', 'definition'. Distinct from the parent document's `domain` field — `domain` is the knowledge area, `chunk_type` is the content structure. |
...
```sql
-- HNSW is preferred over IVFFlat because it does not require training data.
-- IVFFlat created on an empty table would be ineffective until reindexed.
-- HNSW works correctly from the first INSERT and does not need reindexing.
...
| ---------- | ------------ | ------------- | --------------------------------- |
| id         | UUID         | PK            | Primary key                       |
| user_id    | UUID         | FK → users.id, ON DELETE CASCADE | Conversation owner. Cascade: deleting a user removes all their conversations (and transitively all messages via conversations cascade). |
...
| ------------------------------ | ----------------- | ----- | ------------------------------------------------------------ |
| `cache:query:{md5_hash}`       | JSON string       | 3600s | Cached agent response (hash of user_id + normalized query). Only first-message (context-free) queries are cached. User-scoped to match retrieval isolation. |
...
> **Design Decision — Duplicate / Version Uploads:**
> The system does **not** detect or deduplicate uploads. If a user uploads the same book twice (e.g., Convict Conditioning V3 then V4), both are stored as independent `documents` rows, each with their own chunks and embeddings. This is intentional:
> - **No automatic overwrite**: overwriting risks silent data loss if the user uploaded a different edition by mistake.
> - **No incremental merge**: diff-based merging of PDF content is unreliable and adds significant complexity for minimal benefit.
> - **User responsibility**: the user should manually delete the old version via `DELETE /documents/{id}` before or after uploading the new one. The frontend document panel (Day 9) should make this easy by showing all uploads with filenames and timestamps.
> - **Consequence if not cleaned up**: both versions' chunks are returned by retrieval, which may produce duplicate or contradictory citations. This is a minor quality degradation, not a system error — the LLM can still synthesize a coherent answer.
...
```
// The chat endpoint uses POST, so the browser's native EventSource API
// cannot be used (EventSource only supports GET). Instead, use the
// fetch API with a ReadableStream to consume the SSE stream.

const response = await fetch("/api/v1/chat", {
    method: "POST",
    headers: {
        "Content-Type": "application/json",
        "Authorization": `Bearer ${token}`     // JWT in header, not query param
    },
...
while (true) {
    ...
    for (const line of text.split("\n\n")) {
        ...
        switch (event.type) {
            case "routing":  // → show "正在分析问题... → 训练 Agent"
            case "sources":  // → render source cards (collapsed)
            case "token":    // → append to message bubble
            case "done":     // → finalize UI, enable input
            case "error":    // → show error toast, re-enable input
        }
    }
}
```
...
Request: multipart/form-data
  file: (PDF binary)
  domain: "training"    # optional: training | rehab | nutrition
...
class AgentState(TypedDict):
    ...
    user_id: str
    session: AsyncSession              # DB session for retrieval (passed through, not serialized)
...
Edge case rules:
  ...
  - If query is unrelated to fitness → route to "training"; the router sets
    refined_query to the original query unchanged. The training agent's system
    prompt includes a fallback instruction: "If the retrieved context is not
    relevant to the user's question, politely explain that this system
    specializes in fitness topics and offer to help with fitness-related
    questions instead." No special state field is needed — the agent naturally
    handles low-relevance retrieval results via its system prompt.
```
    ...
    for page_num, page_text in raw_pages:
        ...
        if page_text.strip():
            ...
            for split in text_splits:
                classified_type = classify_content(split)  # text | exercise | definition
                chunks.append(Chunk(
                    document_id = document_id,
                    content = split,
                    chunk_index = chunk_index,
                    chunk_type = classified_type,
                    metadata = {
                        "page_start": page_num,
                        "page_end": page_num,
...
async function retrieve(
    ...
    Results are scoped to the requesting user's documents via JOIN."""

    # 1. Embed the query
    query_embedding = await embed_text(query)

    # 2. Build SQL with user isolation and optional metadata filter
    sql = """
        SELECT
            dc.id,
            dc.content,
            dc.chunk_type,
            dc.metadata,
            1 - (dc.embedding <=> :query_vec) AS relevance_score
    ...
    params = {"query_vec": query_embedding, "top_k": top_k, "user_id": user_id}

    if content_domain:
        sql += " AND dc.metadata->>'content_domain' = :domain"
...
async function training_agent_node(state: AgentState) -> AgentState:
    ...
    # 1. Retrieve relevant chunks (user-scoped via JOIN to documents)
...
class CacheService:
    ...
    def _make_key(self, query: str, user_id: str) -> str:
        """Cache key includes user_id because retrieval is user-scoped.
        Two users asking the same question will search different document
        sets and should receive different cached responses."""
    ...
    async def set(self, query: str, user_id: str, response: dict):
        key = self._make_key(query, user_id)
...
async def chat(
    ...
    # Always return SSE format for consistency — client uses fetch-based
    # streaming (not EventSource) since this is a POST endpoint.
...
async function generate_stream(request, user, session, cache):
    ...
    # Cache check — only for first message in a conversation (no history context).
    # Multi-turn queries are NOT cached because identical queries in different
    # conversation contexts should produce different answers.
    is_first_message = len(history) <= 1    # only the message we just saved

    if is_first_message:
        cached = await cache.get(request.message, str(user.id))
        if cached:
            await cache.redis.incr("cache:stats:hits")
            # Wrap cached response in SSE events so the client always receives
            # the same format regardless of cache hit/miss.
            yield f"data: {json.dumps({'type': 'routing', 'agent': cached['agent_used'], 'refined_query': request.message, 'cached': True})}\n\n"
            yield f"data: {json.dumps({'type': 'sources', 'chunks': cached['sources']})}\n\n"
            yield f"data: {json.dumps({'type': 'token', 'content': cached['response']})}\n\n"
            yield f"data: {json.dumps({'type': 'done', 'agent_used': cached['agent_used'], 'latency_ms': 0, 'conversation_id': str(conversation.id), 'cached': True})}\n\n"

            await save_message(conversation.id, "assistant", cached["response"], session,
                               agent_used=cached["agent_used"], sources=cached["sources"], latency_ms=0)
            return
    ...
    # Step 2: Retrieve
    chunks = await retrieve(routing_result.refined_query, routing_result.agent, str(user.id), session=session)
    ...
    # Cache the response — only for first-message (context-free) queries
    if is_first_message:
        await cache.set(request.message, str(user.id), {
...
| Rate Limiting      | 20 requests/minute per user via Redis fixed-window counter. Note: a user could theoretically send up to 40 requests in a 2-second window spanning two minute boundaries. This is an acceptable trade-off for implementation simplicity; upgrade to sliding-window (sorted set) if stricter enforcement is needed. |
...
```
cache:query:{md5(user_id + ":" + normalized_query)}
...
Scoping rules:
  - Cache is user-scoped because retrieval is user-scoped — each user
    searches only their own documents, so the same query from two users
    produces different results
  - Only first-message (context-free) queries are cached — multi-turn
    queries are excluded because conversation history changes the answer
...
async function health_check():
    ...
    # LLM Provider — use a minimal chat completion as the probe.
    # models.list() is not supported by all providers (e.g., DeepSeek, LiteLLM).
    # A single-token completion is the most reliable cross-provider health check.
    try:
...
async def test_cache_miss_then_hit(redis_client):
    ...
    # Miss
    result = await cache.get("test query", user_id)
    assert result is None

    # Set
    await cache.set("test query", user_id, {"response": "cached answer"})

    # Hit
    result = await cache.get("test query", user_id)
    assert result["response"] == "cached answer"

async def test_cache_is_user_scoped(redis_client):
    """Same query from different users should not share cache."""
    cache = CacheService(redis_client, ttl=60)

    await cache.set("如何练引体向上", "user-A", {"response": "answer for A"})
    result = await cache.get("如何练引体向上", "user-B")
    assert result is None   # user-B should not see user-A's cached response

async def test_cache_key_normalization(redis_client):
    cache = CacheService(redis_client, ttl=60)
    user_id = "test-user-uuid"

    await cache.set("  Hello World  ", user_id, {"data": 1})
    result = await cache.get("hello world", user_id)
...
services:
  postgres:
    ...
    # Use `make db-shell` to access via docker compose exec.
    volumes:
  ...
  # Dedicated test database — runs on a different port to avoid
  # conflicts with the main database. Used by `make test`.
  postgres-test:
    image: pgvector/pgvector:pg16
    container_name: fitcoach-db-test
    environment:
      POSTGRES_USER: fitcoach
      POSTGRES_PASSWORD: fitcoach_dev
    ...
    profiles: ["test"]      # Only starts when running `docker compose --profile test up`
    healthcheck:
      test: ["CMD-SHELL", "pg_isready -U fitcoach"]
      interval: 5s
      timeout: 5s
  ...
  redis:
    ...
    # No host port mapping — only accessible within Docker network.
    # Use `make redis-cli` to access via docker compose exec.
    volumes:
      - redisdata:/data
...
| Docker Desktop VM    | ~1.5 GB      | Docker Desktop for Mac runs a Linux VM; typical consumption is 1–2 GB, not the 500 MB often assumed |
| **Remaining buffer** | **~2.1 GB**  | Available for spikes                                         |

> **Note:** The buffer is tighter than it appears. If memory pressure occurs, reduce PostgreSQL `shared_buffers` (default 128 MB is fine for this scale) and consider lowering the backend limit to 768 MB. Monitor with `make stats` (`docker stats`) during development.
...
| ----- | ------------------------------------ | ------------------------------------------------------------ |
| Day 1 | Project scaffolding + Docker Compose | `docker compose up` starts postgres (pgvector), redis, and an empty FastAPI app. `GET /health` returns `200`. Project directory structure matches Appendix A. Makefile shortcuts work (`make up`, `make down`, `make logs`). |
...
│   │   │   ├── user.py
│   │   │   ├── document.py
│   │   │   ├── document_chunk.py    # Separate file: vector column, JSONB metadata, HNSW index
│   │   │   ├── conversation.py
│   │   │   └── message.py
│   │   │
...
# Utilities & HTTP client (also used as async test client by pytest)
httpx>=0.27.0
